...
func main() {
	...
	fmt.Println("====================================================")
	//getSignature(userList[0])
	vmForGoja()
}
func getSignature(userId string) (signature string) {
	vm := otto.New()
	_, err := vm.Run(script)
	if err != nil {
		panic(err)
	}
	v, err := vm.Call("generateSignature", nil, userId)
	if err != nil {
		panic(err)
	}
...
func vmForGoja() {
	vm := goja.New()
	vm.Set("userId", "12345")
	_, e := vm.RunScript("", script)
	if e != nil {
		panic(e)
	}
	signature := vm.Get("signature").String()
	fmt.Println(signature)
}

var testScript = `
var signature = userId + "11111"
`
